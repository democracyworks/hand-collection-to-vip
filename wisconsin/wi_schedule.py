...
#                df['start_date'] = df['start_date'].str.replace('\n', '')
    ...
    def get_start_time(self, index, office_name, start_time, start_date, hours_open_id):
        ...
        #        return st
        #    elif len(start_time.replace(' - ', '-')) == 18:
        #            st = start_time[-3]
    ...
    def build_schedule_txt(self):
        ...
        self.base_df['is_or_by_appointment'] = self.base_df.apply(
            lambda row: self.is_or_by_appointment(row['is_or_by_appointment']), axis=1)
    ...
    def write_schedule_txt(self):
        ...
        sch.start_time.fillna('')  # replace 'nan' with empty string
        #print sch.columns
        # reorder columns
        ...
        sch.drop(['county', 'official_name', 'phone', 'email', 'ocd_division', 'phone', 'location_name', 'address1',
         'address2', 'city', 'state', 'zip_code','directions', 'notes', 'index', 'address_line', 'hours',
         'photo_uri', 'is_drop_box', 'is_early_voting', 'latitude', 'longitude',
         'latlng_source'], inplace=True, axis=1)


        sch = sch.reindex(columns=cols)
        sch.replace(['None', 'nan'], np.nan, inplace=True)
        #print sch
...
if __name__ == '__main__':
    ...
    #s ='office_name, official_name, phone, email, ocd_division, phone, location_name, address1, address2, city, state, zip_code, start_time, end_time, start_date, end_date, is_only_by_appointment, is_or_by_appointment, directions, index, address_line, hours, photo_uri, hours_open_id, is_drop_box, is_early_voting, latitude, longitude, latlng_source, id'.split(', ')
    #print s
    colnames = ['county', 'official_name', 'phone', 'email', 'ocd_division', 'phone', 'location_name', 'address1',
