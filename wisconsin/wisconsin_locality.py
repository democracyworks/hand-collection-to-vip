...
import re
import config
from wi_polling_location import PollingLocationTxt
...
#        return pd.concat(g for _, g in frame.groupby("external_identifier_value") if len(g) > 1)
 ...
 #       loc = loc.groupby('external_identifier_value').agg(lambda x: ' '.join(set(x))).reset_index()
        ...
        loc['name'] = loc.apply(
            lambda row: self.create_name(row['grouped_index'], row['city']), axis=1)

 #       loc['id'] = loc.apply(
 #           lambda row: self.create_id(row['grouped_index']), axis=1)

        #print loc
        loc.drop(['county', 'official_name', 'phone', 'email', 'ocd_division', 'phone', 'location_name', 'address1', 'address2', 'city', 'state', 'zip_code',
         'start_time', 'end_time', 'start_date', 'end_date', 'is_only_by_appointment', 'is_or_by_appointment', 'directions', 'notes', 'index',
         'address_line', 'hours', 'photo_uri', 'hours_open_id', 'is_drop_box', 'is_early_voting', 'latitude', 'longitude', 'latlng_source'], inplace=True, axis=1)
        ...
        drop = loc.drop_duplicates(subset=['polling_location_ids'])
        final = drop.reindex(columns=cols)
...
if __name__ == '__main__':
    ...
    print s
    #colnames = ['county', 'official_name', 'phone', 'email', 'ocd_division', 'phone', 'location_name',
    ...
    #usecols = ['county', 'official_name', 'phone', 'email', 'ocd_division', 'phone', 'location_name',
    #            'address1', 'address2', 'city', 'state', 'zip_code', 'start_time', 'end_time', 'start_date',
    ...
    early_voting_df = pd.read_csv(config.input + config.state_file, names=colnames, sep=',', encoding='ISO-8859-1', skiprows=1)
