...
import csv
import config
from nm_polling_location import PollingLocationTxt
import pandas as pd
...
class ElectionTxt(object):
    ...
    def absentee_request_deadline(self, index):
        ...
        for index, row in self.state_feed.iterrows():
            if row['office_name'] == config.state:
                ...
                return datetime.datetime.strptime(deadline, "%m/%d/%Y").strftime("%Y-%m-%d")
        else:
            print 'Missing value at row '  + str(index) + '.'
            return ''
    ...
    def write(self):
        ...
        et.drop(['name', 'loc_name', 'adr_1', 'adr_2', 'adr_3', 'city', 'state', 'zip', 'county', 'dirs', 'services', 'start_date', 'end_date',
                'start_time', 'end_time', 'day_times', 'st_id', 'comments', 'index', 'address_line', 'directions',
