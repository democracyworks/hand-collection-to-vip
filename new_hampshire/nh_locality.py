...
#    early_voting_df['index'] = early_voting_df.index +1 # offsets zero based index so it starts at 1 for ids
    ...
    usecols = ['county', 'official_title', 'ocd_division', 'homepage_url', 'phone', 'email', 'street', 'city',
                'state', 'zip_code', 'start_time', 'end_time', 'start_date', 'end_date']


    early_voting_df = pd.read_csv(config.input_path + config.state_file, names=colnames, usecols=usecols, encoding='utf-8', skiprows=1)
