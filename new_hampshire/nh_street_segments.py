...
class GenerateStreetSegDependencies(object):
    ...
    # TODO: move methods to module level
    ...
    def groupby_precinct(self):
        ...
        #grouped_by_precinct.to_csv(config.output + 'check_precinct_group1.txt', index=False, encoding='utf-8')
        ...
        #grouped_by_precinct['merge_key'] = 'pre' + grouped_by_precinct['van_precinctid']
...
class StreetSegmentsTxt(object):
    ...
    def includes_all_addresses(self, vf_reg_cass_street_name, vf_reg_cass_city):
        ...
        b = a.shape
        #print b
        print vf_reg_cass_street_name + ',', vf_reg_cass_city + ',' + 'Count is ' + str(b)
        ...
        if b.count == 1:
            ...
            print 'false'
            return 'false'
        #precinct_ids.append(a.van_precinctid_x)
    ...
    def get_end_house_number(self, vf_reg_cass_street_num_x):
        ...
        return vf_reg_cass_street_num_x.split(',')[-1]
    ...
    def build_precinct_txt(self):
        ...
        self.base_df['includes_all_addresses'] = self.base_df.apply(
            lambda row: self.includes_all_addresses(row['vf_reg_cass_street_name'], row['vf_reg_cass_city_x']), axis=1)
...
#        prt = pt.drop(['van_precinctid_x', 'vf_reg_cass_street_name', 'vf_reg_cass_street_num_x', '0', 'merge_key',
#                       'vf_reg_cass_city', 'vf_reg_cass_state', 'vf_reg_cass_zip', 'vf_reg_cass_street_num_y',
#                       'vf_reg_cass_pre_directional', 'vf_reg_cass_street_suffix', 'van_precinctid_y',
#                       'election_administration_id', 'external_identifier_value', 'name', 'id', 'number',
#                       'index'], axis=1).compute()

        prt = pt.drop(pt.columns[[i for i in range(18)]], axis=1)
        ...
        prt.to_csv(config.output + 'street_segments6.txt', index=False, encoding='utf-8')  # send to txt file
...
def main():
    ...
    voter_file_ddf['merge_key'] = voter_file_ddf['vf_township']
    ...
    #precinct_text_ddf.to_csv(config.output + 'precinct_for_streets.txt', index=False, encoding='utf-8')
    ...
    street_seg_merged_df = group_by_precinct.merge(precinct_text_ddf, left_on=['merge_key'], right_on=['van_precinctid'], how='outer')
    ...
    sseg_m.to_csv(config.output + 'street_seg_merge_deduped9.txt', index=False, encoding='utf-8')
