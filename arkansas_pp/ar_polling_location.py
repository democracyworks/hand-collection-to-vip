...
class PollingLocationTxt(object):
    ...
    def get_hours(self, index, start_time, end_time):
        """Convert time from 24 to 12 hour format. Combines start and end time values."""
    ...
    def create_hours_open_id(self, index, address_1, city, state, zip_code):
        """Creates the hours_open_id by prepending 'ho' to a hash of the address_line created in
        ...
        hours_open = int(hashlib.sha1(address_line).hexdigest(), 16) % (10 ** 8)

        return 'ho' + str(hours_open)
    ...
    def create_id(self, index, ocd_division, address_1, city, state, zip_code):
        ...
        created in the get_address_line method.
        """
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['county', 'ocd_division', 'phone', 'email', 'precinct', 'directions', 'name', 'address1', 'address2',
                'city', 'state', 'zip_code', 'start_time', 'end_time', 'index'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    # Columns names from polling_place data as they'll be used in PollingLocationTxt().
