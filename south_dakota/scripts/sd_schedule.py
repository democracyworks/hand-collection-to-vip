...
mountain_tz = ['bennett', ' butte', ' corson', ' custer', ' dewey', ' fall river', ' haakon', ' harding', ' jackson',
 ' lawrence', ' meade', ' pennington', ' perkins', ' rapid city', ' shannon',
 'stanley', ' ziebach']
...
#        return month + '/' + day + '/' + year
    ...
    def get_end_date(self, end_date, start_time):
        ...
        #utc_offset = tuple(start_time.split(' - '))[1]  # take utc_offset value from second index position
        #utc_offset = '0' + utc_offset
    ...
    def build_schedule_txt(self):
        ...
        self.base_df['start_time'] = self.base_df.apply(
            lambda row: self.get_start_time(row['index'], row['source_start_time'], row['source_end_date'], row['county']), axis=1)

        self.base_df['end_time'] = self.base_df.apply(
            lambda row: self.get_end_time(row['source_end_time']), axis=1)
        ...
        self.base_df['start_date'] = self.base_df.apply(
            lambda row: self.get_start_date(row['source_start_date'], row['source_start_time']), axis=1)

        self.base_df['end_date'] = self.base_df.apply(
            lambda row: self.get_end_date(row['source_end_date'], row['source_start_time']), axis=1)
