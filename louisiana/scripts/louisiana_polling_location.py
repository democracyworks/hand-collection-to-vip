...
class PollingLocationTxt(object):
    ...
    def get_address_line(self, index, name, address_one, address_two, city, zip_code):
        ...
        if not pd.isnull(city):
            city_name = str(city).lower().title()
        else:
            city_name = ''
        ...
        line = adr.lower().title().strip() + ", " + city_name + ", LA " + zip
        print line
        return line
    ...
    def create_id(self, index, name, address_one, address_two, city, zip_code):
        """Create id"""
    ...
    def build_polling_location_txt(self):
        ...
        self.base_df['address_line'] = self.base_df.apply(
            lambda row: self.get_address_line(row['index'], row['name'], row['adr_1'],
                                              row['adr_2'], row['city'], row['zip']), axis=1)
        ...
        self.base_df['hours_open_id'] = self.base_df.apply(
            lambda row: self.create_hours_open_id(row['index'], row['name'],row['adr_1'], row['adr_2'], row['city'],
                                              row['zip']), axis=1)
        ...
        self.base_df['id'] = self.base_df.apply(
            lambda row: self.create_id(row['index'], row['name'],row['adr_1'],
                                       row['adr_2'], row['city'], row['zip']), axis=1)
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['name', 'adr_location_name', 'adr_1', 'adr_2', 'adr_3', 'city', 'state', 'zip', 'dirs', 'services',
                'start_date', 'end_date', 'start_time', 'end_time', 'ev_id', 'locality_id', 'index'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    #drop_box_true =
    state_file='louisiana_early_voting_info.csv'
    ...
    colnames = ['name', 'adr_location_name', 'adr_1', 'adr_2', 'adr_3', 'city', 'state', 'zip', 'dirs', 'services',
                'start_date', 'end_date', 'start_time', 'end_time', 'ev_id', 'locality_id']
    print len(colnames)


    early_voting_df = pd.read_csv(open(early_voting_file, "rU"), names=colnames, encoding='utf-8', skiprows=1)
