...
class LocalityTxt(object):
    ...
    def get_external_identifier_value(self, external_identifier_value):
        ...
        # ocd-division/country:us/state:me/place:blue_hill

        county = external_identifier_value.lower().replace(" ", "_")
        county = county.replace(".", "")
        string = "ocd-division/country:us/state:" + config.state_lower_abbreviation +"/parish:" + county.lower()
        return string
    ...
    def build_locality_txt(self):
        ...
        self.base_df['external_identifier_value'] = self.base_df.apply(
            lambda row: self.get_external_identifier_value(row['name']), axis=1)

        self.base_df['name'] = self.base_df.apply(
            lambda row: self.create_name(row['index'], row['name']), axis=1)
    ...
    def write_locality_txt(self):
        ...
        loc.drop(['ocd-division', 'county', 'name', 'address_one', 'address_two', 'city', 'state', 'zip', 'start_time',
                'end_time', 'start_date', 'end_date', 'appt_1', 'appt_2', 'appt_3', 'subject_to_change', 'index', 'address_line', 'directions',
