...
import re
import config
from nueces_polling_location import PollingLocationTxt
...
#            lambda row: self.create_name(row['index'], row['division_description']), axis=1)
        ...
        self.base_df['merge_key'] = self.base_df.apply(
            lambda row: self.create_merge_key(row['ocd_division']), axis=1)
...
#        return pd.concat(g for _, g in frame.groupby("external_identifier_value") if len(g) > 1)
 ...
 #               'index'], inplace=True, axis=1)
        ...
        loc['name'] = loc.apply(
            lambda row: self.create_name(row['grouped_index'], row['ocd_division']), axis=1)
