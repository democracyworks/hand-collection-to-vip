...
class PollingLocationTxt(object):
    ...
    def get_address_line(self, index, address_1, address_2, city, state, zip_code):
        """#"""

        if not pd.isnull(address_1):
            #street =  str(street).strip()
            address_1 = ''.join([i if ord(i) < 128 else ' ' for i in address_1.strip()])
        ...
        if not pd.isnull(address_2):
            #street =  str(street).strip()
            address2 = ''.join([i if ord(i) < 128 else '' for i in address_2.strip()])
            address = address_1 + ', ' + address2 + ','
        else:
            #address_2 = ''
            address = address_1
            #raise ValueError('Missing street value at row ' + str(index) + '.')
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['index', 'ocd_division', 'county', 'location_name', 'address_1', 'address_2', 'city', 'state', 'zip_code',
                'start_time', 'end_time', 'start_date', 'end_date'], inplace=True, axis=1)
