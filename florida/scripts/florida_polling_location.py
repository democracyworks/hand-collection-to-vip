...
class PollingLocationTxt(object):
    ...
    def get_address_line(self, index, address_one, city, zip_code):
        ...
        line = adr.strip() + ", " + city_name.strip() + ", FL " + zip
        return line.strip()

    def get_directions(self, address1, address2, dirs):
        """#"""
    ...
    def create_hours_open_id(self, index, address_one, city, zip_code):
        """#"""
        # TODO: this is the correct id/index code, correct everywhere
        address_line = self.get_address_line(index, address_one, city, zip_code)
        # print address_line
    ...
    def create_id(self, index, ocd_division, address_one, city, zip_code):
        """Create id"""
    ...
    def build_polling_location_txt(self):
        ...
        self.base_df['directions'] = self.base_df.apply(
            lambda row: self.get_directions(row['adr_2'], row['adr_3'], row['dirs']), axis=1)
        ...
        self.base_df['id'] = self.base_df.apply(
            lambda row: self.create_id(row['index'], row['county'], row['adr_1'],
                                    row['city'], row['zip']), axis=1)
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['county', 'site', 'loc_name', 'adr_1', 'adr_2', 'adr_3', 'city', 'state', 'zip', 'dirs', 'services',
                'start_date', 'end_date', 'start_time', 'end_time', 'index'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    #drop_box_true =
    state_file='florida_early_voting_info.csv'
