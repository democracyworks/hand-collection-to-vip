...
class PollingLocationTxt(object):
    ...
    def get_address_line(self, index, name, address, city, zip_code):
        # required: print message for exception
        # TODO: concatenate street, city, state and zip

        print address, city, zip_code
        ...
        return adr.strip() + ", " + city_name + ", WV " + zip
    ...
    def create_hours_open_id(self, index, name, address, city, zip_code):
        """#"""
        # TODO: this is the correct id/index code, correct everywhere
        address_line = self.get_address_line(index, name, address, city, zip_code)
        # print address_line
    ...
    def create_id(self, index, ocd_division, name, address, city, zip_code):
        """Create id"""
    ...
    def build_polling_location_txt(self):
        ...
        self.base_df['address_line'] = self.base_df.apply(
            lambda row: self.get_address_line(row['index'], row['name'], row['street'], row['city'], row['zip']), axis=1)
        ...
        self.base_df['hours_open_id'] = self.base_df.apply(
            lambda row: self.create_hours_open_id(row['index'], row['name'],row['street'], row['city'],
                                              row['zip']), axis=1)
        ...
        self.base_df['id'] = self.base_df.apply(
            lambda row: self.create_id(row['index'], row['ocd-division'], row['name'],row['street'],
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['office-name', 'ocd-division', 'dirs', 'loc_name', 'street', 'city',
                  'state', 'zip', 'start_time', 'end_time', 'start_date', 'end_date', 'notes', 'index'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    #drop_box_true =
    state_file='wv_early_voting_info.csv'
    ...
    string = "office-name,ocd-division,directions,name,street,city,state,zip,start_time,end_time,start_date,end_date,notes"

    colnames = ['office-name', 'ocd-division', 'dirs', 'loc_name', 'street', 'city',
                'state', 'zip', 'start_time', 'end_time', 'start_date', 'end_date', 'notes']
