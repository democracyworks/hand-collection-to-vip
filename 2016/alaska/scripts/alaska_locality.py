...
class LocalityTxt(object):
    ...
    def build_locality_txt(self):
        ...
        self.base_df['external_identifier_value'] = self.base_df.apply(
            lambda row: self.get_external_identifier_value(row['ocd-id']), axis=1)

        self.base_df['name'] = self.base_df.apply(
            lambda row: self.create_name(row['index'], row['ocd-id']), axis=1)
    ...
    def write_locality_txt(self):
        ...
        loc.drop(['office-name', 'title', 'ocd-id', 'description', 'homepage', 'phone', 'email', 'loc_name',
    ...
    def final_build(self):
        ...
        loc = loc.groupby(['ocd-id']).agg(lambda x: ' '.join(set(x))).reset_index()
        # print loc
