...
class ScheduleTxt(object):
    ...
    def build_schedule_txt(self):
        ...
        self.base_df['start_time'] = self.base_df.apply(
            lambda row: self.get_start_time(row['index'], row['municipality'], row['start_time'], row['start_date'],
                                            row['hours_open_id']), axis=1)

        self.base_df['end_time'] = self.base_df.apply(
            lambda row: self.get_end_time(row['municipality'], row['end_time'], row['hours_open_id']), axis=1)
    ...
    def write_schedule_txt(self):
        ...
        sch.drop(['cty', 'municipality', 'wp', 'voting_place', 'street', 'index',
                'address_line', 'directions', 'hours', 'photo_uri', 'is_drop_box', 'is_early_voting',
                'latitude', 'longitude', 'latlng_source', 'source_id'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    colnames = ['cty', 'ocd', 'municipality', 'wp', 'voting_place', 'street', 'start_time', 'end_time', 'start_date', 'end_date', 'index',
                'address_line', 'directions', 'hours', 'photo_uri', 'hours_open_id', 'is_drop_box', 'is_early_voting',
                'latitude', 'longitude', 'latlng_source', 'source_id']
