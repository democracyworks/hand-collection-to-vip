...
from maine_precinct import PrecinctTxt
...
#                pass
    ...
    def write(self):
       ...
       # sseg = sseg[sseg['precinct_id'].isin(['prenan']) == False]
        ...
        # Drop rows where the zip code is missing
        #sseg = sseg[sseg.zip.notnull()]
...
def main():
    ...
    #polling_place_df = polling_place_df[polling_place_df.street.notnull()]
    #polling_place_df = polling_place_df[polling_place_df.city.notnull()]
    #polling_place_df = polling_place_df[polling_place_df.precinct.notnull()]
    ...
    voter_file_df['merge_key'] = voter_file_df['vf_township'] + '-' + voter_file_df['vf_precinct_id']
    del df
    ...
    #merged_df.to_csv(config.output + 'merge_test.txt', index=False, encoding='utf-8')
    merged_df['index'] = merged_df.index + 1
    #merged_df = merged_df.drop_duplicates(subset=['vf_reg_cass_city', 'vf_reg_cass_street_name'])
    print merged_df



    st = StreetSegmentsTxt(merged_df)
    st.write()
