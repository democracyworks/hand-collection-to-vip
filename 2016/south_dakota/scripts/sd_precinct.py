...
def main():
    ...
    df = pd.read_csv(voter_file, sep='\t', names=colnames,
...
#        print merged_df
        ...
        self.base_df['external_identifier_value'] = self.base_df.apply(
            lambda row: self.get_external_identifier_value(row['external_identifier_value_y']), axis=1)
        ...
        self.base_df['locality_id'] = self.base_df.apply(
            lambda row: self.locality_id(row['id_y']), axis=1)
        ...
        self.base_df['polling_location_ids'] = self.base_df.apply(
            lambda row: self.polling_location_ids(row['polling_location_ids_y']), axis=1)
...
#        return pd.concat(g for _, g in frame.groupby("external_identifier_value") if len(g) > 1)
    ...
    def write_precinct_txt(self):
        ...
        pt.drop(['index', 'vf_source_state', 'vf_county_code', 'vf_county_name', 'vf_township', 'vf_ward',
                 'vf_precinct_id', 'vf_precinct_name', 'vf_county_council', 'reg_place_name', 'vf_reg_address_1',
                 'vf_reg_address_2', 'vf_reg_city', 'vf_reg_state', 'vf_reg_zip', 'vf_reg_zip4',
                 ...
                 'van_precinctid', 'election_administration_id_x', 'external_identifier_type_x',
                 'external_identifier_othertype_x', 'external_identifier_value_x', 'name_x', 'polling_location_ids_x',
                 'state_id_x', 'type_x', 'other_type_x', 'id_x', 'merge_key', 'election_administration_id_y',
                 'external_identifier_type_y', 'external_identifier_othertype_y', 'external_identifier_value_y',
                 'name_y', 'polling_location_ids_y', 'state_id_y', 'type_y', 'other_type_y', 'grouped_index',
                 'id_y'], inplace=True, axis=1)

#        pt.drop(pt.columns[[0, 1, 2]], axis=1, inplace=True)
        print pt
