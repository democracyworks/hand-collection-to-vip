...
if __name__ == '__main__':
    ...
    s = 'county, official_title, ocd_division, homepage_url, phone, email, precinct, location_name, address1, directions, city, state, zip_code, start_time, end_time,, index, address_line, hours, photo_uri, hours_open_id, is_drop_box, is_early_voting, latitude, longitude, latlng_source, id'.split(', ')
    print s
    colnames = ['','county', 'official_title', 'ocd_division', 'homepage_url', 'phone', 'email', 'precinct', 'location_name',
                'address1', 'directions', 'city', 'state', 'zip_code', 'start_time', 'end_time,', 'index', 'address_line',
                'hours', 'photo_uri', 'hours_open_id', 'is_drop_box', 'is_early_voting', 'latitude', 'longitude', 'latlng_source',
                'id']

    early_voting_df = pd.read_csv(config.output + intermediate_doc, names=colnames, sep=',', encoding='ISO-8859-1', skiprows=1)

    early_voting_df['index'] = early_voting_df.index # offsets zero based index so it starts at 1 for ids
