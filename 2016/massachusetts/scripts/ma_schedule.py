...
import pandas as pd
import config
from ma_polling_location import PollingLocationTxt
import datetime
import re
...
class ScheduleTxt(object):
    ...
    def get_sch_time(self, hours):
        ...
        if len(hours_arr[0]) < 2:
            hours = "0" + hours
        #print hours, len(hours), mins, len(mins)

        return hours + ":00-04:00"
    ...
    def write_schedule_txt(self):
        ...
        # Drop base_df columns.
        sch.drop(['town', 'loc_name', 'street', 'city', 'zip', 'start_date', 'end_date', 'start_time', 'end_time',
