...
class PollingLocationTxt(object):
    ...
    def get_address_line(self, index, street, city, zip_code):
        ...
        return st.strip() + ", " + city.strip() + ", " + config.state_abbreviation + " " + zip.strip()
    ...
    def build_polling_location_txt(self):
        ...
        self.base_df['hours_open_id'] = self.base_df.apply(
            lambda row: self.create_hours_open_id(row['index'], row['adr_1'], row['city'],
                                              row['zip']), axis=1)
        ...
        self.base_df['id'] = self.base_df.apply(
            lambda row: self.create_id(row['index'], row['county'], row['adr_1'], row['city'], row['zip']), axis=1)
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['name', 'county', 'location', 'adr_1', 'adr_2', 'adr_3', 'city', 'state', 'zip', 'dirs',
                'services', 'start_date', 'end_date', 'state_id', 'start_time', 'end_time', 'index'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    #drop_box_true =
    state_file='maryland_early_voting_info.csv'
    ...
    colnames = ['name', 'county', 'location', 'adr_1', 'adr_2', 'adr_3', 'city', 'state', 'zip', 'dirs',
                'services', 'start_date', 'end_date', 'state_id', 'start_time', 'end_time']
    print len(colnames)
    ...
    # wisco_file = "wisconsin_early_voting_info.csv"
    #
    # wi_file = config.input + wisco_file
    #
    # cols = ['name', 'official', 'phone', 'email', 'ocd-division', 'phone', 'loc_name', 'adr_1', 'adr_2',
    #         'city', 'state', 'zip', 'start_time', 'end_time', 'start_date', 'end_date', 'is_only',
    #         'is_or', 'directions', 'notes']
    # wisco_df = pd.read_csv(wi_file, names=cols, encoding='utf-8', skiprows=1)
    # #wisco_df['index'] = wisco_df.index + 1
    # print wisco_df