...
#        return id
    def get_id(self, ev_id):
        return ev_id

    def get_locality_id(self, locality_id):
        return locality_id
...
#            lambda row: self.create_id(row['index'], row['county'], row['address1'], row['address2'],
#                                              row['city'], row['state'], row['zip_code']), axis=1)

        self.base_df['id'] = self.base_df.apply(
            lambda row: self.get_id(row['ev_id']), axis=1)

        self.base_df['locality_id'] = self.base_df.apply(
            lambda row: self.get_locality_id(row['locality_id']), axis=1)
...
if __name__ == '__main__':
    ...
    s = 'name location_name address_line1 address_line2 address_line3 address_city address_state address_zip directions voter_services start_date end_date start_time end_time days_times_open ev_id locality_id county'.split(' ')
    print s
