...
def main():
    """Reads voter file into a dataframe and passes it to an instance of the StreetSegment class.

    """

    # Voter file columns.
    colnames = ['vf_source_state', 'vf_county_code', 'vf_county_name', 'vf_cd', 'vf_sd', 'vf_hd', 'vf_township', 'vf_ward',
    ...
    df = pd.read_csv(voter_file, sep=',', names=colnames, encoding='ISO-8859-1',
                     skiprows=1,
                     iterator=True,
                     chunksize=1000,
                     dtype='str'
    ...
    pima_voter_file_df = pd.concat(df, ignore_index=True)
    pima_voter_file_df['index'] = pima_voter_file_df.index + 1
    del df

    # Create StreetSegments instance
    st = StreetSegmentsTxt(pima_voter_file_df)
    st.write()
