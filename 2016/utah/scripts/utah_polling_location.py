...
class PollingLocationTxt(object):
    ...
    def build_polling_location_txt(self):
        ...
        self.base_df['address_line'] = self.base_df.apply(
            lambda row: self.get_address_line(row['index'], row['name'], row['address_1'],
                                              row['address_2'], row['city'], row['zip']), axis=1)
        ...
        self.base_df['hours_open_id'] = self.base_df.apply(
            lambda row: self.create_hours_open_id(row['index'], row['loc_name'],row['address_1'], row['address_2'], row['city'],
                                              row['zip']), axis=1)
        ...
        self.base_df['id'] = self.base_df.apply(
            lambda row: self.create_id(row['index'], row['county'], row['name'],row['address_1'],
                                       row['address_2'], row['city'], row['zip']), axis=1)
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['entry_name', 'county', 'loc_name', 'address_1', 'address_2', 'address_3', 'city', 'state', 'zip', 'dirs',
                'services', 'start_time', 'end_time', 'start_date', 'end_date', 'days_times_open', 'state_id', 'index'], inplace=True, axis=1)
...
if __name__ == '__main__':
    ...
    #drop_box_true =
    state_file='utah_early_voting_info.csv'
    ...
    colnames = ['entry_name', 'county', 'loc_name', 'address_1', 'address_2', 'address_3', 'city', 'state', 'zip', 'dirs',
                'services', 'start_time', 'end_time', 'start_date', 'end_date', 'days_times_open', 'state_id']
