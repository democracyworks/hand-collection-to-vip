...
import re
import config
from wa_polling_location import PollingLocationTxt
...
class LocalityTxt(object):
    ...
    def get_external_identifier_value(self, county):
        ...
        if county:
            c = 'ocd-division/country:us/state:' + config.state + '/county:' + county.replace(' County', '')
            print c
            return c
        else:
            return ''
    ...
    def get_polling_location_ids(self, ev_id):
        """
    ...
    def build_locality_txt(self):
        ...
        self.base_df['polling_location_ids2'] = self.base_df.apply(
            lambda row: self.get_polling_location_ids(row['ev_id']), axis=1)
...
#        return pd.concat(g for _, g in frame.groupby("external_identifier_value") if len(g) > 1)
 ...
 #               'index'], inplace=True, axis=1)
        ...
        #loc = loc.groupby('hours_open_id', 'county').agg(lambda x: ' '.join(set(x))).reset_index()
        loc = loc.groupby(['external_identifier_value', 'polling_location_ids']).agg(lambda x: ' '.join(set(x))).reset_index()
        print loc
...
if __name__ == '__main__':
    ...
    s = 'name, location_name, address1, address2, address_line3, city, state, zip_code, directions, voter_services, start_date, end_date, start_time, end_time, days_times_open, ev_id, locality_id, county, index, address_line, hours, photo_uri, hours_open_id, is_drop_box, is_early_voting, latitude, longitude, latlng_source, id'.split(', ')
    print s
