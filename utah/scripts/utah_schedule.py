...
import pandas as pd
import config
from utah_polling_location import PollingLocationTxt
import datetime
import re
...
class ScheduleTxt(object):
    ...
    def get_sch_time(self, hours, start_date):
    ...
    def write_schedule_txt(self):
        ...
        # Drop base_df columns.
        sch.drop(['entry_name', 'county', 'loc_name', 'address_1', 'address_2', 'address_3', 'city', 'state', 'zip', 'dirs',
                'services', 'start_time', 'end_time', 'start_date', 'end_date', 'days_times_open', 'state_id',
                'index', 'name', 'address_line', 'directions',
                'hours', 'photo_uri', 'hours_open_id', 'is_drop_box', 'is_early_voting', 'lat', 'long', 'latlng', 'poll_id'], inplace=True,
                 axis=1)
