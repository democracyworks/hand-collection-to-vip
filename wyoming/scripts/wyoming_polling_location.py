...
class PollingLocationTxt(object):
    ...
    def create_hours_open_id(self, index, address_one, address_two, city, zip_code):
        """#"""
        # TODO: this is the correct id/index code, correct everywhere
        address_line = self.get_address_line(index, address_one, address_two, city, zip_code)
        # print address_line
    ...
    def build_polling_location_txt(self):
        ...
        self.base_df['address_line'] = self.base_df.apply(
            lambda row: self.get_address_line(row['index'], row['address_one'], row['address_two'], row['city'],
                                              row['zip']), axis=1)
        ...
        self.base_df['hours_open_id'] = self.base_df.apply(
            lambda row: self.create_hours_open_id(row['index'], row['address_one'], row['address_two'], row['city'],
                                              row['zip']), axis=1)
        ...
        self.base_df['id'] = self.base_df.apply(
            lambda row: self.create_id(row['index'], row['ocd_division'], row['address_one'], row['address_two'],
    ...
    def write_polling_location_txt(self):
        ...
        plt.drop(['ocd_division', 'email', 'county', 'loc_name', 'address_one', 'address_two', 'city', 'state', 'zip',
...
if __name__ == '__main__':
    ...
    colnames = ['ocd_division', 'email', 'county', 'loc_name', 'address_one', 'address_two', 'city', 'state', 'zip',
                'start_time', 'end_time', 'start_date', 'end_date', 'by_appointment_one', 'by_appointment_two', 'appointmnents',
                'subject_to_change']
    print len(colnames)
